...
sdp.create_streaming_table('streaming_rides')

#Appending bulk or initial load in the Streaming Table 
@sdp.append_flow (
  target = "streaming_rides"
  )
def rides_bulk():
    return (
        spark.readStream.table("uber.bronze.bulk_rides")
        .withColumn(
            "booking_timestamp",
            to_timestamp(col("booking_timestamp"))
        )
    )
